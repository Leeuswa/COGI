...
새 채팅에서 이 폴더를 열면 **이 문서부터 읽는다.**

> 웹(claude.ai/code)에서 이어서 하려면 **12절**을 먼저 봐라. 저장소에 올라가 있어야 원격에서 스킬과 규칙이 따라온다.
...
## 8-2. 아직 커밋 안 한 것 (2026-07-31 세션) — 로그인 전 화면

8-1과 **같은 브랜치에 올린다**(사용자 지시). 브랜치 `feature/plan-register-and-landing`.

**로그인 전 모바일**
- 폰에서 `nav.gnb`가 숨겨지는데 `MobileHeader`가 비로그인이면 `null`이라 **상단이 통째로 비어 있었다.** 로고·뒤로·가입 진입을 넣었다
- `MobileLanding.tsx` 신설 — 한 화면에 카드 둘, `scroll-snap`. 스타일은 `styles/mobile/landing.css`
- 더보기 시트에 **로그아웃** — 데스크톱 GNB의 로그아웃이 폰에서 GNB째로 숨겨져 나갈 길이 없었다

**코기 캐시 계정별 분리** (`GameContext.tsx`)
- 세션 만료 401은 `client.ts:48`에서 **페이지를 통째로 리로드**한다. 그래서 로그아웃 처리가 못 돌고 그 계정 코기가 비로그인 랜딩에 Lv.2로 남았다
- 키를 나눴다 — 비로그인 `cogi-game`, 계정 `cogi-game-{userId}`

**데스크톱 랜딩 개편** (`Landing.tsx`, `styles/landing.css`)
- 섹션 = 한 화면 `scroll-snap`. 양이 적은 성장 루프·케어만 `.lp-duo`로 묶었다 → 5화면
- 마지막 화면에 플랜·CTA·푸터를 함께 담는다. `min-height: calc(100dvh - 76px - 76px)`에서 **빼는 값이 GNB·푸터 실측과 어긋나면 그만큼 밀려 잘린다**
- CTA(남색)는 `flex: 0 0 35%`. 퍼센트가 풀리려면 부모에 `height`가 있어야 한다 — `min-height`만으로는 안 된다
- 구름을 히어로 아래 화면에도(`::before/::after`). 섹션 배경 `sky2`가 불투명해서 **구름을 덮고 있었다** — 배경을 걷어내니 보인다
- 마퀴 삭제. 리드 문구는 쉼표·마침표에서 끊는다(`<br />` 직접)

**랜딩 문구를 실제 동작에 맞게** — 근거는 코드다
- 요금제는 `GET /api/plans`에서 받는다. 랜딩에 박힌 모델명·가격이 실제와 달랐다
- 카테고리 5종 = `IssueCategory` 실제 값(구조 개선·코드 스타일). "가독성"은 없다
- 내보내기 Notion 삭제 — Markdown·PDF만 있다
- 코인 정답 20 / 오답 5 / 출석 10. **연속 학습 보너스는 코드에 없다**
- 주간 리포트 월 10시 → **9시**(`WeeklyReportScheduler:18`)
- 팀 초대는 `repo` 도메인에 플랜 검사가 **0건** — FREE도 된다. "MAX 전용"은 틀렸다
- "AWS Bedrock 기반" 삭제 — 벤더 API를 직접 부른다

**함정** — 카드에 `display: flex; flex-direction: column`을 주면 자식이 가로로 stretch돼 `inline-block` 칩(코인 배지, STEP 배지)이 카드 폭을 꽉 채운다.
...
**지금 남은 건 이 셋뿐이다** (2026-07-31 기준, 사용자 확인)

| 항목 | 내용 | 원격 가능 |
|---|---|---|
| **로그인 후 데스크톱 스냅** | 랜딩과 같은 화면 단위 스냅을 로그인 후 화면에도. `.lp` 스코프라 대시보드는 아직 안 걸려 있다 | ✅ |
| **태블릿 641~1024** | 지금은 데스크톱 규칙이 그대로 걸린다. 모바일 쪽에 맞춰 접어야 한다 | ✅ |
| **375×667 모바일 랜딩 넘침** | 페이지가 85~185px 넘친다. 리뷰 카드 하나가 546px라 카드 둘이 611px에 안 들어간다 — 페이지당 카드 1장으로 되돌리거나 불릿을 줄여야 한다 | ✅ |

"원격 가능"은 claude.ai/code 컨테이너에서 되는지 여부다. 셋 다 프론트라 원격에서 된다.

### 그 밖 (기존 목록)

| 항목 | 담당 | 성격 |
|---|---|---|
| 프롬프트 출력 검수 (모델등급×수준 12개) | 나 | 노가다. **로컬에서만** — 원격엔 DB·백엔드가 없다 |
...
| ~~로그인 전 페이지 모바일 전용~~ | 나 | **완료** (8-2). 모바일 랜딩 신설 + 데스크톱 랜딩 개편 |
...
## 12. 웹(claude.ai/code)에서 이어서 하기

웹 세션은 **GitHub 저장소를 클론한 클라우드 컨테이너**에서 돈다. 로컬 폴더를 보지 않는다.
그래서 저장소에 없는 건 원격에 존재하지 않는다 — 스킬도, 이 문서도.

### 준비 (한 번만)

1. **`.claude/skills` · `.claude/agents` · `.claude/launch.json` 커밋**
   안 올리면 원격엔 `/ponytail`도 `/caveman`도 없다.
   `settings.local.json`은 전역 gitignore(`~/.config/git/ignore`)에 이미 걸려 자동 제외된다 — 개인 권한 목록이라 올리면 안 된다.

2. **`CLAUDE.md` 커밋**
   세션 시작 때 자동으로 읽히는 건 `CLAUDE.md`뿐이다. `HANDOFF.md`는 자동 로드가 아니다.
   그래서 CLAUDE.md에 "0절 두 줄 켜라 + HANDOFF.md 읽어라"를 적어 뒀다.

3. **`HANDOFF.md` 커밋**
   원래 저장소에 안 올리던 문서인데, 원격에서 읽으려면 올려야 한다. 개인 메모라 곤란하면
   대신 CLAUDE.md에 필요한 규칙만 옮겨 적고 HANDOFF는 로컬에 둔다.

4. **브랜치 push** — 원격에 없는 브랜치는 웹에서 고를 수 없다.

### 새 채팅 만들 때

`claude.ai/code` → 저장소 `Leeuswa/COGI` 선택 → **브랜치 선택**(작업 중인 브랜치) → 프롬프트 입력.

첫 프롬프트는 이 정도면 된다.

```
HANDOFF.md 읽고 9절 "지금 남은 건 이 셋뿐이다"부터 이어서 하자.
```

### 원격에서 안 되는 것

컨테이너에 **MySQL도 백엔드 서버도 없다.** 프론트 dev server·빌드·CSS는 되지만 아래는 로컬에서만 된다.

- `plan_started_at` 컬럼 생성 확인
- 프롬프트 실제 출력 검수 (12개)
- 로그인이 필요한 화면 확인 (대시보드·학습카드 등)

9절 표의 "원격 가능" 칸이 그 구분이다. 남은 세 항목은 셋 다 프론트라 원격에서 된다.

### 앱과 웹을 같이 쓸 때

다른 계정·다른 파일을 앱에서 만지는 중이라면 **브랜치를 나눠라.** 같은 브랜치를 양쪽에서 push하면 강제 푸시 사고가 난다.
웹에서 만든 커밋은 원격에 바로 올라가므로, 로컬에서 이어받을 땐 `git pull` 먼저.
