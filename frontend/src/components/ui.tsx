    ...
    <nav className="gnb">
      ...
      {user ? (
        <div className="links">
          ...
          {!user.githubUsername && (
            <Link className="btn sm wh" to="/app/repos">
              🐙 GitHub 연동
            </Link>
          )}
          <button className="btn sm wh" onClick={logout}>
            로그아웃
          </button>
        </div>
      ) : (
        <div className="links">
          ...
          <Link className="btn co sm" to="/signup">
            회원가입
          </Link>
        </div>
      )}
...
export function Footer() {
  return (
    <footer className="ft">
      <div>Team Fable · 정상연 · 이수환 · 홍성찬 · 유지환</div>
...
function Bell({ loginId }) {
  ...
  useEffect(() => {
    api.getNotifications(loginId).then(setList);
  }, [loginId]);
  useEffect(() => {
    // 바깥 클릭으로 닫기
    const close = (e) => {
    ...
    <span className="bell-wrap" ref={boxRef}>
      ...
      {open && (
        <div className="bell-drop">
          {list.length === 0 ? (
            <p className="note sm" style={{ padding: 14 }}>
              새 알림이 없어요.
            </p>
          ) : (
            list.map((n) => (
...
export function GradeLight({ grade }) {
  const map = {
    RED: ["red", "학습 시작"],
    YELLOW: ["yellow", "학습 진행중"],
    GREEN: ["green", "해결"],
    GREEN_PLUS: ["green", "해결+"],
  };
  const [cls, label] = map[grade] ?? map.RED;
  return (
    <span className={`light ${cls}`}>
      <i />
      {label}
    </span>
  );
...
export function TermsDialog({ term, onClose }) {
  ...
  return (
    <div className="modal-mask" onClick={onClose}>
      ...
      >
        <h3>
          {term.title} <span className="chip navy">v{term.version}</span>
        </h3>
        <p style={{ fontSize: 13.5, lineHeight: 2, margin: "14px 0 22px" }}>
          {term.content}
        </p>
        <button className="btn wh sm" onClick={onClose}>
          닫기
        </button>
      </div>
    </div>
