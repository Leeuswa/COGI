...
(() => {
  ...
  ov.id = '__cogi_ov';
  ov.style.cssText = 'position:absolute;pointer-events:none;z-index:99999;display:none;outline:2px dashed #ff6b57;outline-offset:1px';
  const HANDLES = ['nw','n','ne','e','se','s','sw','w'];
  HANDLES.forEach((h) => {
    const d = document.createElement('div');
    d.dataset.h = h;
    d.style.cssText = 'position:absolute;width:10px;height:10px;background:#fff;border:2px solid #1b2a4a;pointer-events:auto;cursor:' + h + '-resize;';
    ov.appendChild(d);
  });
...
const Prop = ({ label, children }) => (
  <label className="prop-row"><span>{label}</span>{children}</label>
);
// 아코디언: 한 번에 한 섹션만 — 열면 다른 섹션은 접힌다
const Sect = ({ id, title, children, openId, setOpenId }) => (
  <details className="prop-sect" open={openId === id}
    onToggle={(e) => { if ((e.currentTarget as HTMLDetailsElement).open && openId !== id) setOpenId(id); }}>
    <summary onClick={(e) => { e.preventDefault(); setOpenId(openId === id ? null : id); }}>{title}</summary>
    {children}
  </details>
);
...
export default function PreviewDock({ code, onCode }) {
  ...
  const pushHist = (c) => {
    ...
    h.stack = h.stack.slice(0, h.idx + 1).concat(c).slice(-50);
  ...
  useEffect(() => {
    const onMsg = (e) => {
      const d = e.data || {};
      if (d.cogi === 'pick') { setPicked(d); setText(d.text ?? ''); }
      if (d.cogi === 'dim') setDim({ w: d.w, h: d.h });
      if (d.cogi === 'clear') { setPicked(null); setDim(null); }
      if (d.cogi === 'rot') setRotVal(d.deg); // 마우스 회전 → 슬라이더 동기화
      if (d.cogi === 'code') { skipNext.current = true; onCode(d.code); pushHist(d.code); }
    };
  ...
  useEffect(() => {
    ...
    setDoc(code); setPicked(null); setDim(null); setRotVal(0);
  }, [code]);

  const apply = (prop: string, value?: any) =>
    frameRef.current?.contentWindow?.postMessage({ cogi: 'apply', prop, value }, '*');
  const addEl = (kind) =>
  ...
  return (
    <div className={`dock ${full ? 'full' : ''}`}>
      ...
      <div className="dock-bar">
        <select className="dock-add" defaultValue="" aria-label="요소 추가"
          onChange={(e) => { if (e.target.value) { addEl(e.target.value); e.target.value = ''; } }}>
          <option value="" disabled>＋ 요소 추가</option>
        ...
        <button className="dock-tool" title="실행취소 (Ctrl+Z)" onClick={() => timeTravel(-1)}>↩</button>
        <button className="dock-tool" title="다시실행 (Ctrl+Shift+Z)" onClick={() => timeTravel(1)}>↪</button>
        <span className="note xs">클릭 선택 · 드래그 이동 · 8점 핸들 크기 · 노란 핸들 회전 · 방향키 이동 · Del 삭제 · Ctrl+D 복제</span>
        <span className="ml-auto" />
        <select className="dock-add" value={zoom} aria-label="줌"
          onChange={(e) => setZoom(Number(e.target.value))}>
          {[50, 75, 100, 125, 150].map((z) => <option key={z} value={z}>{z}%</option>)}
        </select>
        <button className="dock-full-btn" title={full ? '창 모드로 (Esc)' : '전체화면으로'}
          onClick={() => setFull((f) => !f)}>
          {full ? '⤡ 창 모드' : '⤢ 전체화면'}
        </button>
      </div>

      <div className="dock-stage" ref={stageRef}
        style={full ? { gridTemplateColumns: `${cols.code}% 6px 1fr 6px ${cols.props}%` } : undefined}>
          ...
          <>
            <div className="dock-code-wrap side">
              <pre className="dock-code-hl" aria-hidden="true">
                {code.split('\n').map((ln, i) => (
                  <div key={i} className={changed.has(i) ? 'chg' : ''}>{ln || ' '}</div>
                ))}
              </pre>
              <textarea className="dock-code side" value={code} spellCheck={false}
                onChange={(e) => { onCode(e.target.value); pushHist(e.target.value); }}
                aria-label="코드 (실시간 동기화)" />
            </div>
            <div className="dock-resizer" onMouseDown={startResize('code')} title="드래그로 폭 조절" />
          </>
        )}
        <div className="dock-canvas">
          <div className="dock-scale" style={{ width: `${10000 / zoom}%`, height: full ? `${10000 / zoom}%` : undefined, transform: `scale(${zoom / 100})` }}>
            <iframe ref={frameRef} title="라이브 미리보기" className="dock-frame"
              sandbox="allow-scripts allow-same-origin" srcDoc={srcDoc} />
          </div>
        </div>

        {full && <div className="dock-resizer" onMouseDown={startResize('props')} title="드래그로 폭 조절" />}
          ...
          {picked ? (
            <>
              <div className="row" style={{ marginBottom: 10 }}>
                ...
                {dim && <span className="chip gray">{dim.w}×{dim.h}</span>}
              </div>

              <Sect id="text" title="텍스트" openId={openSect} setOpenId={setOpenSect}>
                {picked.text !== null && (
                  <Prop label="내용">
                    <input type="text" value={text}
                      onChange={(e) => { setText(e.target.value); apply('text', e.target.value); }} />
                  </Prop>
                )}
                <Prop label="글꼴">
                  <select defaultValue="inherit" onChange={(e) => apply('fontFamily', e.target.value)}>
                    {FONTS.map(([v, ko]) => <option key={ko} value={v}>{ko}</option>)}
                  </select>
                </Prop>
                <Prop label="크기">
                  <input type="number" min="8" max="120" defaultValue={st.fontSize}
                    onChange={(e) => apply('fontSize', e.target.value + 'px')} />
                </Prop>
                <Prop label="굵기">
                  <select defaultValue={st.fontWeight >= 600 ? 'bold' : 'normal'}
                    onChange={(e) => apply('fontWeight', e.target.value)}>
                    <option value="300">얇게</option><option value="normal">보통</option>
                    <option value="bold">굵게</option><option value="900">아주 굵게</option>
                  </select>
                </Prop>
                <Prop label="색">
                  <input type="color" defaultValue={st.color} onChange={(e) => apply('color', e.target.value)} />
                </Prop>
                <Prop label="정렬">
                  <span className="btn3">
                    <button onClick={() => apply('textAlign', 'left')}>⇤</button>
                    <button onClick={() => apply('textAlign', 'center')}>≡</button>
                    <button onClick={() => apply('textAlign', 'right')}>⇥</button>
                  </span>
                </Prop>
                <Prop label="꾸밈">
                  <span className="btn3">
                    <button style={{ fontStyle: 'italic' }} onClick={() => apply('fontStyle', 'italic')}>I</button>
                    <button style={{ textDecoration: 'underline' }} onClick={() => apply('textDecoration', 'underline')}>U</button>
                    <button style={{ textDecoration: 'line-through' }} onClick={() => apply('textDecoration', 'line-through')}>S</button>
                  </span>
                </Prop>
                <Prop label="자간">
                  <input type="number" step="0.5" min="-3" max="24" defaultValue={st.letterSpacing}
                    onChange={(e) => apply('letterSpacing', e.target.value + 'px')} />
                </Prop>
                <Prop label="줄간격">
                  <input type="number" step="0.1" min="1" max="3" defaultValue="1.6"
                    onChange={(e) => apply('lineHeight', e.target.value)} />
                </Prop>
              </Sect>

              <Sect id="layout" title="배치" openId={openSect} setOpenId={setOpenSect}>
                <Prop label="너비">
                  <input type="number" min="0" max="1600" defaultValue={dim?.w}
                    onChange={(e) => apply('width', e.target.value ? e.target.value + 'px' : 'auto')} />
                </Prop>
                <Prop label="높이">
                  <input type="number" min="0" max="1200" defaultValue={dim?.h}
                    onChange={(e) => apply('height', e.target.value ? e.target.value + 'px' : 'auto')} />
                </Prop>
                <Prop label="안쪽 여백">
                  <input type="number" min="0" max="160" defaultValue={st.padding}
                    onChange={(e) => apply('padding', e.target.value + 'px')} />
                </Prop>
                <Prop label="바깥 여백">
                  <input type="number" min="0" max="160" defaultValue={st.margin}
                    onChange={(e) => apply('margin', e.target.value + 'px')} />
                </Prop>
                <Prop label="회전(°)">
                  <span className="rot-ctrl">
                    <input type="range" min="-180" max="180" value={rotVal}
                      onChange={(e) => { setRotVal(Number(e.target.value)); apply('rot', Number(e.target.value)); }} />
                    <input type="number" min="-180" max="180" value={rotVal} className="rot-num"
                      onChange={(e) => { const v = Number(e.target.value) || 0; setRotVal(v); apply('rot', v); }} />
                  </span>
                </Prop>
                <Prop label="크기 맞춤">
                  <button className="fit-btn" title="상자를 내용 길이에 딱 맞게"
                    onClick={() => apply('fit')}>내용에 맞춤</button>
                </Prop>
                <Prop label="정렬·겹침">
                  <span className="btn3">
                    <button title="가로 가운데" onClick={() => apply('centerX')}>▣</button>
                    <button title="한 층 앞으로" onClick={() => apply('layer', 'up')}>⬆</button>
                    <button title="한 층 뒤로" onClick={() => apply('layer', 'down')}>⬇</button>
                  </span>
                </Prop>
                <Prop label="복제·삭제">
                  <span className="btn3">
                    <button title="복제 (Ctrl+D)" onClick={() => apply('dup')}>⧉</button>
                    <button title="삭제 (Del)" onClick={() => apply('del')}>🗑</button>
                  </span>
                </Prop>
              </Sect>

              <Sect id="style" title="스타일" openId={openSect} setOpenId={setOpenSect}>
                <Prop label="배경색">
                  <input type="color" defaultValue={st.background} onChange={(e) => apply('background', e.target.value)} />
                </Prop>
                <Prop label="배경 이미지">
                  <input type="text" placeholder="이미지 URL"
                    onKeyDown={(e) => { if (e.key === 'Enter') apply('bgImage', (e.target as HTMLInputElement).value); }}
                    onBlur={(e) => e.target.value && apply('bgImage', e.target.value)} />
                </Prop>
                <Prop label="그라데이션">
                  <select defaultValue="" onChange={(e) => e.target.value && apply('background', e.target.value)}>
                    {GRADS.map(([v, ko]) => <option key={ko} value={v}>{ko}</option>)}
                  </select>
                </Prop>
                <Prop label="테두리 굵기">
                  <input type="number" min="0" max="12" defaultValue={st.borderW}
                    onChange={(e) => apply('border', Number(e.target.value) ? `${e.target.value}px solid ${st.borderC}` : 'none')} />
                </Prop>
                <Prop label="테두리 색">
                  <input type="color" defaultValue={st.borderC}
                    onChange={(e) => apply('border', `${st.borderW || 2}px solid ${e.target.value}`)} />
                </Prop>
                <Prop label="둥글기">
                  <input type="number" min="0" max="100" defaultValue={st.radius}
                    onChange={(e) => apply('borderRadius', e.target.value + 'px')} />
                </Prop>
                <Prop label="그림자">
                  <select defaultValue="" onChange={(e) => apply('boxShadow', e.target.value)}>
                    ...
                    <option value="inset 0 3px 8px rgba(0,0,0,.25)">안쪽</option>
                  </select>
                </Prop>
                <Prop label="투명도(%)">
                  <input type="range" min="10" max="100" defaultValue={st.opacity}
                    onChange={(e) => apply('opacity', Number(e.target.value) / 100)} />
                </Prop>
              </Sect>

              <Sect id="fx" title="효과 (필터)" openId={openSect} setOpenId={setOpenSect}>
                <Prop label="블러">
                  <input type="range" min="0" max="12" defaultValue="0" onChange={(e) => apply('blur', Number(e.target.value))} />
                </Prop>
                <Prop label="밝기(%)">
                  <input type="range" min="40" max="180" defaultValue="100" onChange={(e) => apply('bright', Number(e.target.value))} />
                </Prop>
                <Prop label="대비(%)">
                  <input type="range" min="40" max="180" defaultValue="100" onChange={(e) => apply('contrast', Number(e.target.value))} />
                </Prop>
                <Prop label="채도(%)">
                  <input type="range" min="0" max="200" defaultValue="100" onChange={(e) => apply('sat', Number(e.target.value))} />
                </Prop>
                <Prop label="흑백(%)">
                  <input type="range" min="0" max="100" defaultValue="0" onChange={(e) => apply('gray', Number(e.target.value))} />
                </Prop>
              </Sect>
            </>
          ) : (
            <p className="note sm" style={{ lineHeight: 2 }}>
              미리보기에서 요소를 <b>클릭</b>해 선택하세요.{'\n'}
              드래그로 옮기고, 테두리의 8개 점을 잡으면 크기가 바뀝니다.\n노란 동그라미를 돌리면 회전(Shift=15° 스냅).{'\n'}
              방향키 미세이동 · Del 삭제 · Ctrl+D 복제 · Ctrl+Z 실행취소.
            </p>
          )}
        </aside>
      </div>
      ...
      {!full && (
        <div className="dock-code-wrap">
          ...
          <textarea className="dock-code" value={code} spellCheck={false}
            onChange={(e) => { onCode(e.target.value); pushHist(e.target.value); }}
