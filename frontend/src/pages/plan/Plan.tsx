  ...
  const confirm = async () => {
    ...
    try {
      if (paying.name === 'FREE') {
        ...
        notify(`플랜 전환 완료. 차액 ${res.proratedAmount?.toLocaleString() ?? 0}원 (테스트 계산)`);
      }
      patchUser({ planName: paying.name });
      setHistory((h) => [
        { id: h.length + 1, previousPlan: user.planName, newPlan: paying.name,
          changeType: paying.name === 'FREE' ? 'CANCEL' : user.planName === 'FREE' ? 'START' : 'CHANGE',
          proratedAmount: 0, changedAt: new Date().toISOString() },
        ...h,
      ]);
      setPaying(null);
      setCardNo('');
  ...
  return (
    <main className="app-main">
      ...
      <div className="panel-grid c3">
        {plans.map((p) => {
          const mine = user.planName === p.name;
          return (
            <div key={p.id} className={`panel plan-card ${mine ? 'current' : ''}`}>
              {mine && <div className="plan-flag">현재 이용 중</div>}
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                <b style={{ fontFamily: 'Silkscreen, monospace', fontSize: 16 }}>{p.name}</b>
              </div>
              <p style={{ fontSize: 22, margin: '10px 0 2px' }}>
                {p.price === 0 ? '₩0' : `₩${p.price.toLocaleString()}`}<span className="note xs">/월</span>
              </p>
              <p style={{ fontSize: 12.5, color: 'var(--sub)', marginBottom: 12 }}>
                {p.model} · 일 {p.dailyCreditLimit}크레딧
              </p>
              <ul style={{ listStyle: 'none', fontSize: 12.5, lineHeight: 2, marginBottom: 16 }}>
                {p.features.map((ft) => <li key={ft}>• {ft}</li>)}
              </ul>
              <button className={`btn ${mine ? 'wh' : 'co'} sm`} style={{ width: '100%' }}
                disabled={mine} onClick={() => setPaying(p)}>
                {mine ? '사용 중' : p.name === 'FREE' ? '해지하고 FREE로' : '이 플랜으로'}
              </button>
            </div>
          );
        })}
      ...
      {paying && (
        <div className="panel" style={{ outline: '3px solid var(--coral)' }}>
          <h3>💳 {paying.name} {paying.name === 'FREE' ? '전환(해지)' : '결제'} — 테스트 모드</h3>
          <p className="note" style={{ marginBottom: 14 }}>
            실제 결제가 일어나지 않는 테스트 환경입니다. 아무 숫자나 넣어도 통과돼요. (FR-86)
          </p>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button className="btn co sm" onClick={confirm} disabled={busy || (paying.name !== 'FREE' && !payAgree)}>
              {busy ? '처리 중…' : '확정'}
            </button>
            <button className="btn wh sm" onClick={() => { setPaying(null); setPayAgree(false); }}>취소</button>
          </div>
        </div>
      )}
      ...
      <div className="panel flush">
        <table className="tbl">
          <thead>
            <tr><th>변경</th><th>이전</th><th>이후</th><th>차액</th><th>일시</th></tr>
          </thead>
          <tbody>
            {history.map((h) => (
              <tr key={h.id}>
                <td><span className={`chip ${h.changeType === 'CANCEL' ? 'gray' : 'navy'}`}>{h.changeType}</span></td>
                <td className="mono">{h.previousPlan}</td>
                <td className="mono">{h.newPlan}</td>
                <td className="mono">{h.proratedAmount?.toLocaleString() ?? 0}원</td>
                <td className="mono xs">{h.changedAt.slice(0, 16).replace('T', ' ')}</td>
              </tr>
            ))}
          </tbody>
        </table>
